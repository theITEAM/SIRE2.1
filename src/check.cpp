...
void MCMC::check_chain(unsigned int num) {
	...
	if (different(prior, prior_check) == true)
		emsg("prior problem");
	prior = prior_check;
	...
	for (auto g = 0; g < model.ngroup; g++) {
		if (different(L_inf_events[g], L_inf_events_check[g], SMALLISH) == true) {
			cout << num << "num\n";
			cout << "\nL_inf_events[" << g << "] = " << L_inf_events_check[g]
				<< ", L_inf_events_check[" << g << "] = " << L_inf_events[g]
				<< "  "<< L_inf_events[g]-L_inf_events_check[g] << " "
				<< endl;
			emsg("L_inf_events problem");
		}
	}
	...
	if (different(L_trans_events, L_trans_events_check) == true){
		cout << L_trans_events << " " << L_trans_events_check << "j\n";
		emsg("L_trans_events problem");
	}
	...
	for (auto g = 0; g < model.ngroup; g++) {
		if (different(L_diag_test[g], L_diag_test_check[g]) == true)
			emsg("L_diag_test problem");
	}
	...
	for (auto i = 0; i < model.N; i++) {
		...
		if (ind.infected == false) {
			for (auto tr = 0; tr < model.ntrans; tr++) {
				if (ind.trans_time[tr] != UNSET)
					emsg("Trans time should be unset");
			}
		} else {
			for (auto tr = 1; tr < model.ntrans; tr++) {
				if (ind.trans_time[tr] < ind.trans_time[tr - 1])
					emsg("Trans time in wrong order");
			}
	...
	for (auto g = 0; g < model.ngroup; g++) {
		if (std::isnan(L_inf_events[g]) || std::isinf(L_inf_events[g]))
			emsg("L_inf_events is not a number");
	}

	if (std::isnan(L_trans_events) || std::isinf(L_trans_events))
		emsg("L_trans_events is not a number");

	for (auto g = 0; g < model.ngroup; g++) {
		if (std::isnan(L_diag_test[g]) || std::isinf(L_diag_test[g]))
			emsg("L_diag_test is not a number");
	}
...
void check_ind_effect_sample(const Model &model) {
	// cout << "Checking ind_effect_sampler" << endl; // DEBUG
...
void MCMC::temp() {
	...
	*/
	...
	for (auto k = 0; k < ind_list.size(); k++) {
		auto i = ind_list[k];
