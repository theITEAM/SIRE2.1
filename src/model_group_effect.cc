...
void Model::propose_group_effect_sigma(vector <double> &param_value, double &prior, vector <Jump> &param_jump, const bool burnin) const
{
  ...
  for(auto loop = 0; loop < group_effect_sigma_prop; loop++){
    ...
    auto al = exp(prior_propose - prior);
...
void Model::propose_group_effect(const vector <IndValue> &ind_value, vector <double> &param_value, vector <double> &L_inf_events, double &prior, vector <Jump> &param_jump, const bool burnin) const
{
  for(auto g = 0u; g < ngroup; g++){
    ...
    for(auto loop = 0; loop < group_effect_prop; loop++){
      ...
      auto al = exp(L_propose - L_inf_events[g] + prior_change);
...
PrecalcGroupEffect Model::set_precalc_group_effect(const Group &gr, const vector <IndValue> &ind_value, const vector <double> &param_value) const
{
  ...
  for(const auto &ev : event){
    ...
    if(tr == 0){
      if(tev > tmin){ prec.log_beta_num++; prec.con_fac += log(beta*ind.susceptibility*I);}
