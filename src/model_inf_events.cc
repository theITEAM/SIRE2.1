...
void Model::set_individual_quantities(vector <IndValue> &ind_value, const vector <double> &param_value) const
{
  ...
  for(auto &ind : ind_value){
    ...
    for(auto tr = 0u; tr < ntrans; tr++){       
      ...
      auto sum = 0.0;
      for(auto ie : tra.ind_effect) sum += ind.ind_effect[ie];
      for(auto fe : tra.fixed_effect) sum += indiv.fixed_effect_X[fe]*fixed_effect_value[fe];
			for(auto se : tra.snp_effect){
				switch(indiv.SNP_genotype[se]){
					case AA: sum += snp_effect_value[se]; break;
					case AB: sum += snp_effect_dom[se]; break;
					case BB: sum -= snp_effect_value[se]; break;
				}
			}
    ...
    for(auto c = 0u; c < ncomp; c++){                            // This sets individual-based infectivity 
      auto inf = comp[c].infectivity;
      if(inf != 0){
        auto sum = 0.0; 
        for(auto ie : comp[c].ind_effect) sum += ind.ind_effect[ie];
        for(auto fe : comp[c].fixed_effect) sum += indiv.fixed_effect_X[fe]*fixed_effect_value[fe];
    ...
    if(false){
      ...
      for(auto tr = 1u; tr < ntrans; tr++) cout << ind.trans_mean[tr] << " ";
      cout << "   Transition infectivity change: ";
      for(auto tr = 0u; tr < ntrans; tr++) cout << ind.trans_infectivity_change[tr] << " ";
      cout << endl;
    }
  }
}
...
bool EV_ord (Event ev1, Event ev2){ return (ev1.time < ev2.time); }; 
  ...
  for(auto i : gr.ind_ref){
    ...
    if(ind.infected == true){
      for(auto tr = 0u; tr < ntrans; tr++){
        ...
        if(t < tmax){
          Event ev; ev.ind = ind.index; ev.trans = tr; ev.time = t;
          event.push_back(ev);
        }
      }
    }
...
double Model::likelihood_inf_events(const Group &gr, const vector <IndValue> &ind_value, const vector <double> &param_value) const
{
  ...
  if(t > tmax) emsg("Likelihood error");
  L -= beta*S*I*(tmax-t);
...
void Model::propose_transmission_rate(const vector <IndValue> &ind_value, vector <double> &param_value, vector <double> &L_inf_events, double &prior, vector <Jump> &param_jump) const
{
  timer[TIME_TRANS_RATE].start();
  
  if(trans[0].type != INFECTION) emsg("Transition should be infection");
  ...
  for(auto g = 0u; g < ngroup; g++){
    ...
    auto fac = 1.0; if(inf_model == FREQ_DEP) fac /= gr.nind;         // Add frequency dependence
    if(group_effect.on == true){                                      // Adds in the group effect
    ...
    for(const auto &ev : event){
      ...
      if(tr == 0){
        if(tev > tmin){ prec.log_beta_num++; prec.con_fac += log(fac*ind.susceptibility*(I+EXT_FOI));} 
        S -= ind.susceptibility;
      }
  ...
  if(false){   // This checks that Gibbs sampling is being performed correctly
    ...
    auto Li = 0.0; for(auto g = 0u; g < ngroup; g++) Li += L_inf_events[g];
    ...
    cout << exp(Lp-Li +probfi-probif) << "al" << endl;
  }
...
void Model::set_initial_events(vector<IndValue> &ind_value, const vector <double> &param_value) const
{
  for(auto i = 0u; i < N; i++){
    ...
    do{
			...
			if(loop > 0.5*INITIAL_TRANS_SAMPLE) ev_samp = SAMPLE_MIX;
			if(loop > 0.8*INITIAL_TRANS_SAMPLE) ev_samp = SAMPLE_RANGE; 

      if(event_sample(ind_value[i],param_value,ev_samp) != -LARGE) break;
      loop++;
    }while(loop < INITIAL_TRANS_SAMPLE);
		
    if(loop == INITIAL_TRANS_SAMPLE) emsg("Cannot find initial sequence"); 
  }
}


/// Samples a possible set of events for each individual
double Model::event_sample(IndValue &ind, const vector <double> &param_value, EvInitSampType ev_samp) const 
{
  auto probif = 0.0;
  ...
  switch(indiv.status){
    case INFECTED:
      ...
      for(auto tr = 0u; tr < ntrans; tr++){
        ...
        if(trange.tmin != trange.tmax){
          switch(trans[tr].type){
            case INFECTION:
              tr_time = trange.tmin + ran()*(trange.tmax - trange.tmin);
              break;
              
            case GAMMA:
              {
                auto mean = ind.trans_mean[tr];
                auto shape = param_value[trans[tr].shape_param];
                auto dt = gamma_sample(mean,shape);
								...
								switch(ev_samp){
									...
									case SAMPLE_MIX: 
										if(ran() < 0.5) tr_time = trange.tmin + dt;
										else tr_time = ind.trans_time[tr-1] + dt;
										break;
                ...
                if(tr_time < ind.trans_time[tr-1]) return -LARGE;
                if(tr_time <= trange.tmin || tr_time >= trange.tmax) return -LARGE;
    ...
    case NOT_INFECTED:
      ind.infected = false;
      for(auto tr = 0u; tr < ntrans; tr++) ind.trans_time[tr] = UNSET;
      break;
...
void Model::propose_event_times(vector <IndValue> &ind_value, const vector <double> &param_value, vector <double> &L_inf_events, double &L_trans_events, vector <double> &L_diag_test, vector <Jump> &event_jump, const bool burnin) const
{
  ...
  for(const auto &gr : group){
    if(gr.nev_change > 0){
        ...
        if(ind.infected == true){
          ...
          switch(type){
            case PEAKED_SAMPLE:
              if(tr == 0){                          // This uses the next transition time and samples backwards in time
                ...
                switch(trans[tr_next].type){
                  case GAMMA:
                    {
                      auto shape = param_value[trans[tr_next].shape_param];
                  ...
                  default: emsg("Not supported"); break;
                }
              }
                ...
                if(tr == ntrans-1){
                  ...
                  auto tr_next = tr+1;
                  if(trans[tr].type != GAMMA || trans[tr+1].type != GAMMA) emsg("Problem");
                  
                  auto mean1 = ind.trans_time[tr-1] + ind.trans_mean[tr];
                  auto var1 = mean1*mean1/param_value[trans[tr].shape_param];
                  
                  auto mean2 = ind.trans_time[tr+1] - ind.trans_mean[tr+1];
                  auto var2 = mean2*mean2/param_value[trans[tr+1].shape_param];
            ...
            case UNIFORM_SAMPLE:                            // A transtion time is randomly selected from the possible range
              ...
              if(tr > 0){ 
                auto t_last = ind.trans_time[tr-1];
                if(t_last > tmin) tmin = t_last;
              }
              
              if(tr < ntrans-1){
                auto t_next = ind.trans_time[tr+1];
                if(t_next < tmax) tmax = t_next;
              }
          ...
          auto fail = false;                                          
          if(t_prop <= tran.tmin || t_prop >= tran.tmax) fail = true;  // The resample fails because out of time range
          if(tr > 0){ if(t_prop <= ind.trans_time[tr-1]) fail = true;} // Fails because time ordering incorrect
          if(tr < ntrans-1){ if(t_prop >= ind.trans_time[tr+1]) fail = true;}
          
          jump.nevent_tr++;
          if(fail == true) jump.nevent_fa++;
          else{
            ...
            TimeChange tch; tch.ind = i; tch.trans = tr; tch.time = t;
      ...
      if(false) cout << L_trans_events_prop - L_trans_events + probfi - probif << " shoudl be zero" << endl;
			
      auto al = exp(L_inf_events_prop - L_inf_events[g] +  L_diag_test_prop - L_diag_test[g]
        			    + L_trans_events_prop - L_trans_events + probfi - probif);
      ...
      if(ran() < al){
        ...
        if(burnin == true){ jump.size *= prop_up; if(jump.size > gr.nev_change) jump.size = gr.nev_change;}
      }
      else{
...
{
	auto z = ran();
	auto b = 0u; while(b < nbin && z > prob_sum[b]) b++;
...
double InfSampler::sample_prob(double t)
{
	...
	if(b < 0 || b >= nbin) emsg("Sampler out of range");
	return log_prob[b];
}
...
void Model::propose_add_rem(vector <IndValue> &ind_value, const vector <double> &param_value, vector <double> &L_inf_events, double &L_trans_events, vector <double> &L_diag_test, vector <Jump> &add_rem_jump, const bool burnin) const
{
    ...
    if(gr.nunknown > 0){
      ...
      for(auto loop = 0u; loop < loop_max; loop++){
        ...
        for(auto i : gr.unknown){
          if(ind_value[i].infected == true) infected.push_back(i);
          else uninfected.push_back(i);
        }
        ...
        if(ran() < 0.5){                                             // Add a new infected
          if(nuninfected > 0){
            ...
            if(fail == false){
              ...
              probif += ind.inf_sampler.sample_prob(t_inf);  
				      ...
				      uninfected.erase(uninfected.begin()+j); nuninfected--;
              infected.push_back(i); ninfected++;
          ...
          if(ninfected > 0){
            ...
            infected.erase(infected.begin()+j); ninfected--;
            uninfected.push_back(i); nuninfected++;
            ...
            for(auto tr = 0u; tr < ntrans; tr++) trans_time[tr] = UNSET;
        ...
        if(false){                                            // Checks lists are updated correctly
          if(ninfected != infected.size()) emsg("error1");
          if(nuninfected != uninfected.size()) emsg("error2");
          if(ninfected+nuninfected != gr.nunknown) emsg("error3");
          
          for(auto i : gr.unknown){
            if(ind_value[i].infected == true){
              if(find_in(infected,i) == UNSET) emsg("error4");
            }
            else{
              if(find_in(uninfected,i) == UNSET) emsg("error5");
            }
          }
        }
        ...
        if(addrem_change.size() > 0){
          ...
          if(ran() < al){
            ...
            L_trans_events = L_trans_events_prop; 
						L_diag_test[g] = L_diag_test_prop;						
            if(burnin == true){ jump.size *= prop_up; if(jump.size > gr.nunknown*2) jump.size = gr.nunknown*2;}
          }
          else{
