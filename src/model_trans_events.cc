...
void Model::propose_trans_params(vector <IndValue> &ind_value, vector <double> &param_value, double &L_trans_events, double &prior, vector <Jump> &param_jump, const bool burnin) const
{
  ...
  for(auto loop = 0; loop < trans_param_prop; loop++){
    for(auto th = 0; th < nparam; th++){
      if(param[th].type == TRANS_MEAN || param[th].type == TRANS_SHAPE){
        ...
        auto al = exp(L_propose - L_trans_events + prior_change);
...
vector <PrecalcTransParam> Model::set_precalc_trans_param(const vector <IndValue> &ind_value, vector <double> &param_value) const
{
  ...
  for(auto tr = 1u; tr < ntrans; tr++){
    ...
    prec.shape_fac = 0; prec.shape_over_mean = 0; prec.shape_log = 0; prec.lgamma_fac = 0; prec.con_fac = 0;
