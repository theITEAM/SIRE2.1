...
*/
  ...
  */
  ...
  for(const auto &gr : model.group){
    ...
    if(model.inf_model == FREQ_DEP) beta /= N;                      // If frequency dependent divided by group size
    ...
    for(auto i : gr.ind_ref){
      if(model.individual[i].initial_comp != 0){
        ...
        if(c != 0){
          add_infected(0,i,c);
          for(auto tr = 0u; tr < c; tr++) I += ind_value[i].trans_infectivity_change[tr];
        }
      }
    }
    ...
    do{
      ...
      for(auto j = 0u; j < N; j++){
        ...
        if(ind.infected == false) S += ind.susceptibility;
        else{
          auto tr = 0u; while(tr < model.ntrans && ind.trans_time[tr] <= t) tr++;
          if(tr < model.ntrans){
      ...
      double tt = LARGE;
      if(S > 0 && I > TINY) tt = t + exp_sample(beta*S*I);             // Samples time of next infection event
      
      if(next_time >= tmax && tt >= tmax) break;                  // Stops when final time reached
      
      if(tt < next_time){ 
        auto z = ran()*S;
        auto j = 0u; while(j < N && z > S_st[j]) j++;
        if(j == N) emsg("Problem");
...
void Simulate::add_infected(const double t_inf, const int i, const int comp_init)
{
  ...
  for(auto tr = 0u; tr < comp_init; tr++){
...
{
  auto th = 0u; while(th < model.nparam && model.param[th].name != name) th++;
  if(th == model.nparam) emsg("Cannot find '"+name+"'");
...
void Simulate::output_datatable()
{
  ...
  auto num_infected = 0u;
  for(auto i = 0u; i < model.N; i++){
    auto &indiv = model.individual[i];
    auto &ind = ind_value[i];
    if(ind.infected == true) num_infected++;
  }
  ...
  for(auto i = 0u; i < model.N; i++){
    ...
    if(model.nfixed_effect > 0){
      ...
      fout << "\t";
			switch(indiv.SNP_genotype[0]){
				case AA: fout << "AA"; break;
				case AB: fout << "AB"; break;
				case BB: fout << "BB"; break;
			}
	  }
    ...
    if(indiv.initial_comp ==  UNSET) fout << "NA";
      ...
      for(auto t = 0; t < 20; t += 2){
        auto c = indiv.initial_comp;
        auto tr = 0u; while(tr < model.ntrans &&  ind.trans_time[tr] < t){ c = model.trans[tr].to; tr++;}
        if(t != 0) fout << ",";
        fout << "[";
...
void Simulate::output_pedigree()
{
	...
	auto Ainv = model.invert_matrix(A);
	
	fout << "<pedigree>" << endl;
	for(auto j = 0u; j < model.N; j++){
		vector <int> par;
		for(auto i = 0u; i < model.N; i++){
			if(Ainv[j][i] != 0){
				if(j > i) par.push_back(i);
		...
		if(par.size() == 0) fout << ".\t.";
		if(par.size() == 1) fout << model.individual[par[0]].id << "\t.";
		if(par.size() == 2) fout << model.individual[par[0]].id << "\t" << model.individual[par[1]].id;
		if(par.size() > 2) emsg ("Parent problem");
		fout << endl;
	}
	fout << "</pedigree>" << endl;
}
...
void Simulate::output_matrix() const 
{
  ...
  for(auto j = 0u; j < model.N; j++){   
    for(auto i = 0u; i < model.N; i++){
      if(M[j][i] < 1.001 && M[j][i] > 0.9999) M[j][i] = 1;
      if(M[j][i] < 0.501 && M[j][i] > 0.4999) M[j][i] = 0.5;
      if(M[j][i] < 0.2501 && M[j][i] > 0.24999) M[j][i] = 0.25;
    }
  }
  ...
  for(auto j = 0u; j < model.N; j++){   
    for(auto i = 0u; i < model.N; i++){
      if(M[j][i] != 0) fout << j << "\t" << i << "\t" << M[j][i] << endl; 
    }     
  }
  fout << "</A_nonzero>" << endl;
  ...
  for(auto j = 0u; j < model.N; j++){ 
    ...
    for(auto val: model.matrix[1].Ainvlist[j]) fout3 << val.i << "," << val.val << "  ";
    fout3 << endl;
  }
}
